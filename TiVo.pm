...
## TiVo::Server->new( % )
##
##  Constructor for TiVo::Server. Accepts parameters via arguement
##  hash.
##    SERVER_NAME
##    CACHE_DIR
...
## TiVo::Server->command_QUERYSERVER( $ )
##
...
## TiVo::Server->command_QUERYCONTAINER( $ )
##
...
sub command_QUERYCONTAINER {
    ...
    if ( defined( $params->_Recurse ) && uc( $params->_Recurse ) eq 'YES' ) {
        ...
        unless ( $params->_Container eq '/Shuffle' ) {
            @list = sort {
                return -1
                  if ( ref $a ) =~ /^TiVo::Container/
                  && ( ref $b ) =~ /^TiVo::Item/;
                return 1
                  if ( ref $b ) =~ /^TiVo::Container/
                  && ( ref $a ) =~ /^TiVo::Item/;
...
sub explode {
    ...
    @$list = sort {
        return -1
          if ( ref $a ) =~ /^TiVo::Container/ && ( ref $b ) =~ /^TiVo::Item/;
        return 1
          if ( ref $b ) =~ /^TiVo::Container/ && ( ref $a ) =~ /^TiVo::Item/;
        return uc( $a->_Path ) cmp uc( $b->_Path );
...
##############################################################################
# TiVo::Item #   Attaches TiVo methods to a particular file
##############################################################################
package TiVo::Item;
@ISA = ('TiVo');
